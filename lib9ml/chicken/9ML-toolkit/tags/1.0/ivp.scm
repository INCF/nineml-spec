...
(define opt-grammar
  `(
    ...
    (plot            "make plots of simulation data in files ${OPERAND}_NAME.png"
		     (single-char #\p))
	     ...
	     (let ((sd       (construct (car diagram+initial)))
	       ...
	       (let* ((dfe (dataflow sd '()))
		      (dvars (lset-difference eq?
					      ...
					      (list ivar)))
		      (pvars (lset-difference eq? (alist-ref 'in dfe) dvars)))


		 (case platform
		   
		   ((octave/mlton)
		    (ivp-octave/mlton prefix ivp-id ivar dvars pvars start end ic sd)
...
(define (generate-ivp-plot prefix  ivp-info)
  (let ((ivp-id (car ivp-info))
    ...
    (let* ((n (+ 1 (length dvars)))
      ...
      (run (octave -q --eval 
		   ...
		   h = plot ("log(:,1)" #\, ,(sprintf "log(:,2:~A)" n)) #\; 
...
(define (make-ivp-hook #!key (ivp #f) (diagram #f) (ivp-plot #f))
  (lambda (prefix name label value)
	...
	(if ivp-plot (generate-ivp-plot prefix ivp-info))
...
(define (interpreter operand)
  (let ((defs (parse 'NineML (open-input-file operand))))
...
(define (main options operands)
  ...
  (if (null? operands)
      ...
      (let ((output-type (cond ((options 'output-xml)  'xml)
			       ...
			       (else #f)))
	    (unified-envs (map interpreter operands)))
	 ...
	 (lambda (operand uenv)
	   (let ((source-defs (car uenv))
	     ...
	     (cond ((options 'html-report)
		   ...
		   ((options 'plot)
		    (traverse-definitions operand uenv value-hook: (make-ivp-hook ivp: #t ivp-plot: #t)))
