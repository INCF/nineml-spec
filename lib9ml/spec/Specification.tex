...
In the NineML Abstraction Layer the model is defined in terms of {\tt
  ComponentClass}es. A {\tt ComponentClass} is the conglomerating
object which is the subject of interaction with the User Layer.  A
{\tt ComponentClass} could typically represent a point-neuron model or
a synapse model for example. It is composed of an {\tt Interface} and
its {\tt Dynamics}.
...
\begin{itemize}
\item {\tt StateVariable} (section~\ref{state-var})
\item A {\tt RegimeGraph} (section~\ref{regime-graph}) composed of
  instances of {\tt Regime} and {\tt Transition}
\item {\tt Alias} (section~\ref{alias})
\end{itemize}
...
instances of {\tt Port} (section~\ref{ports}) and {\tt Parameter}
...
The internal state of a component is defined by a set of {\tt StateVariable}
...
\textsl{dX/dt=1-X}
\item discretely through {\tt StateAssignment} (in {\tt Transition}),
which make discrete changes to a {\tt StateVariable} value, for example
\textsl{X = X + 1}
\end{itemize}
\end{quote}
...
The {\tt Regime} forms the vertices and the {\tt Transition} form the
directional edges of the {\tt Regime Graph}, which must have at least
one {\tt Regime}, and contain no regime islands.  At any given time, a
component will be in a single {\tt Regime}, and can change which {\tt
  Regime} it is in through {\tt Transition}.
...
on {\tt StateVariable}.  As such, {\tt Regime} defines how the {\tt
  StateVariable}s change (propagate in time) between subsequent
\nmlClass{Transition}. {\tt Regime} is defined to have non-vanishing
temporal extent. Once construction of the {\tt Regime} is complete, it
...
\item a set of {\tt TimeDerivative}, one for each {\tt StateVariable}
of the {\tt ComponentClass}, which define how the {\tt StateVariable}
evolve over time in the form \textsl{dx\_i/dt = f(x\_0, ..., x\_i, t)}.
\item {\tt Parameter} for the \nmlClass{Regime}
...
Movement between several instances of {\tt Regime} happens via {\tt
...
in Figure~\ref{SimpleRegimeGraph}). There are two types of {\tt Transition}:

\begin{itemize}
\item An {\tt OnCondition} function of the {\tt StateVariable}, for
example \textsl{X>Y}.
...
\item The component can send {\tt OutputEvent}
\end{itemize}

During a transition, multiple {\tt StateAssignment} and {\tt OutputEvent} can occur. 
(For more on the resolution of {\tt Transition}, see section~\ref{resolution})
...
triggers the \nmlClass{Transition}
\item An set of {\tt StateAssignment} and{\tt OutputEvent} objects
...
\begin{itemize}
\item Rather than writing long expressions for functions of {\tt StateVariable},
we can define an {\tt Alias} once. For example, we can define chains of
\textbf{Aliases}:
%
...
\note{{\tt Alias} is defined in the Dynamics, \emph{not} in the
...
AnalogPorts transmit and receive continuous values, either {\tt Alias}
or {\tt StateVariable}. {\tt AnalogPort} can have 3 modes:
\begin{itemize}
...
differ from {\tt RecvPort} in that they can be connected to multiple
{\tt SendPort}. {\tt ReducePort} take an additional operator,
{\tt reduce\_op}, which specifies how the data from multiple {\tt SendPort}
should be combined to produce a single value. Currently, the
only supported operations is \textsl{+}, which sums the inputs.
The motivation for {\tt ReducePort} is that it allows us to make our
component definitions more general. For example, if we are defining a
neuron, would define a {\tt ReducePort} called {\tt InjectedCurrent}.
...
Abstraction Layer {\tt Parameter} and combines a value of one of the data
