...
\newpage
\vskip 1in
\paragraph{Changes from version 0.99-3:}
\begin{itemize}
\item Added object definition tables to Interface, ComponentClass, and
  Alias sections (Ivan Raikov)
\end{itemize}

\paragraph{Changes from version 0.99-2:}
\begin{itemize}
\item Add text from Robert on the Unit and Dimension. Modified it to be
compatible with the current agreement about the UL tags.
...
\begin{itemize}
\item Added object definition tables in the abstraction layer section
(Ivan Raikov)
\end{itemize}

\paragraph{Changes from version 0.99:}
\begin{itemize}
\item Reorganization of the document based on discussion during the documentation
meeting:
...
the "Dimension" element with attributes for each of the base dimensions, e.g. :
...
and Volts (multiplier = 1). Units can be expressed in a NineML file with
the "Units" element containing the symbol, a reference to the dimension,
and the numerical value of the multiplier, as in:
...
To express a dimensional quantity both a numerical factor and a unit are
required. These can be  expressed in the User Layer, as follows:
...
and such references only occur on the right hand side of parameter
assignments. Internally, dimensional quantities are to be understood
...
A \ComponentClass can be used to either a specific model of neuron or a
...
A \ComponentClass is the subject of interaction with the User Layer and
is composed of a \Dynamics block and \Interfaces to communicate between
each other, as shown in figure~\ref{fig:EX1_RegimeGraph}.
...
An example model of an Izhikevich model is given for the Abstraction
Layer as:
%
\begin{lstlisting}
