...
As one of the goals of NineML is to provide a mean to import/export a model from any simulator or implementation, it is crucial to maintain a clear distinction between the definition of NineML and a simulator. NineML should provide only the information necessary for any given simulator
...
but not how to solve it.  In addition, for implementation and performance reasons, it is important to keep the language layer "close'' to the simulator – such that the language layer is not responsible for maintaining separate representations of the instantiated network. 
...
\begin{enumerate}
\item a description of the Abstraction and User Layer Object model
...
[TO DO: reformulate the definition of the dynamic to integrate the concepts of regime graph]
...
What is the definition of an alias? Is it equivalent to the previous binding ?
