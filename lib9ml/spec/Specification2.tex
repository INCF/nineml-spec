...
Description of the OM classes are reorganize to ``fit'' the hierarchical structure
in XML both for the AL and the UL.
...
This is the root namespace tag for a NineML file. It can contain
\verb|<component>| elements. (In the examples on the web it also can
have \verb|<import>| elements to bring in external files. Are these
supported in this release? AG.)
...
\issue{Some other data types can be used too, do we use any in this release? AG.}
...
\begin{itemize}
\item An unordered collection of {\tt Transition}s which connect the {\tt
Regime}s
...
\item at least one {\tt Regime} in the {\tt Container} or its sub-{\tt
Container}s.
