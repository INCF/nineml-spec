...
\noindent
\begin{footnotesize}
Acknowledgments: We would like to thank the Task Force members for their
...
[Proposed alternative introduction by Ivan:
The abstraction layer is meant to contain mathematical abstractions
such as functions, differential equations, event handling, and other
concepts necessary to define the dynamics of neuron models, synapse
models, and synaptic/intrinsic plasticity. ]

[Ivan: I think the following  paragraph does not make sense, best to
rewrite it from scratch]

The Object Model for the Abstraction Layer can be represented by a graphical
visualization of the different components and their inter-relations.
...
[Comment by Ivan: the following two paragraphs are not very clear. It
is misleading to say that the ComponentClass represents all the
entities of a network model. It was originally intended to serve as a
generic container. So I suggest having one paragraph along the
following lines:

The ComponentClass object is intended to package together a collection
of objects that relate to the definition of neuronal or synaptic
dynamics. All equations and event declarations that are part of the
neuronal model belong in a single ComponentClass.]

The main building block of the Object Model is the \ComponentClass.
The \ComponentClass object is intended to package together a collection of objects that relate to the definition of all the individual entities existing in a network model (cells, synapses, synaptic plasticity rules, random spike trains, inputs, ...). All equations and event declarations that are part of particular entity model, such as neuron model for instance, belong in a single ComponentClass.
...
implementation] [I agree, it does not fit here. AG]
...
the dynamics. [TO DO: rewrite to incorporate the term substitution and
...
parameters, see section~\ref{dimensions}). It defines particular aspects
of the model, such as
the firing threshold, reset voltage, the decay time constant of a
synapse model.  The instantiation of Parameters at the Abstraction
Layer level allow us to define the dynamics of a general component.
It is then possible to instantiate several variant of this component by attributing different values for the \Parameters, in the User Layer.
For example, if we are building an integrate-and-fire neuron, we can
specify that the Reset-Voltage and the Firing-Threshold are
parameters, write our dynamics in terms of these parameters, then use
the User Layer to provide parameters to create different neurons. By
definition, Parameters are set at the start of the simulation, and
...
\protect\caption{ComponentClass Overview (Detailed)}
\label{fig:EX2_RegimeGraph}
\end{figure}
...
[COMMENT YANN: Should we keep this section?]

A projection is a User Layer object that holds a description of the connectivity
...
These tags specify where the variable is calculated (\verb|<from>|) and
...
\newpage
\section{Identifier Scope: Scoping rule?}
\label{scope}

[COMMENT YANN: This section is difficult to understand...we should rewrite some part to make it clear]
...
[COMMENT YANN: Where are they defined ?]
...
Hierarchical Namespace scheme used to unambiguously address elements
of a given NineML model.  We then proceed to define the elements
which populate Namespaces to make up an Abstraction Layer model and
...
NineML standard library (see Section \ref{nml_lib}).
...
computational element of NineML, referred to as \emph{values}, we
...
have Namespace as value and eliminate NameEntry altogether. AG]
...
Regime and/or discontinuous changes in Regime state variables, a
...
exposing globally a subset of frequently occurring standard constants
