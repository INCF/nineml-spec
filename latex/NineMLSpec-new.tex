...
\subsubsection{Units attribute}
Each Property element requires a \emph{units} attribute. The \textit{units} attribute specifies the units of the quantity and should refer to the name of a Unit element in the document scope. For a dimensionless units a Unit\withelemDimension with no SI dimensions can be used. The SI dimensions of the Unit\withelemDimension should match the SI dimensions of the corresponding Parameter\withelemDimension.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Child elements} & \em{Multiplicity} & \em{Required} \\
    \midrule
    Component{}\withelemRandomDistribution | Reference{}\pointtoComponent{}\withelemRandomDistribution & singleton & yes \\
    \bottomrule
  \end{edtable}
\end{table}

RandomValue elements represent arrays of values drawn from random distributions, which are defined by a Component{}\withelemRandomDistribution elements. The size of the generated array is determined by the size of the container (i.e. Population or Projection) the RandomValue is nested within.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Child elements} & \em{Multiplicity} & \em{Required} \\
    \midrule
    Component{}\withelemDynamics | Reference{}\pointtoComponent{}\withelemDynamics & singleton & yes \\
    \bottomrule
  \end{edtable}
\end{table}
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Child elements} & \em{Multiplicity} & \em{Required} \\
    \midrule
    Reference{}\pointtoPopulation | Reference{}\pointtoSelection & singleton & yes \\
...
The Response defines the effect on the post-synaptic cell dynamics of an incoming synaptic input. The additional dynamics are defined by a Component\withelemDynamics element, which can be defined inline or referenced. For static connections (i.e. those without a Plasticity element), the magnitude of the response (i.e. synaptic weight) is typically passed as a property of the Response element.

The post-synaptic response dynamics can receive incoming port connections from the plasticity rule (see FromPlasticity) or the pre or post synaptic populations (see FromSource and FromDestination). The post-synaptic response object is implicitly created and connected to these ports if the Connectivity\withelemConnectionRule determines that the source and destination cells should be connected.
...
Each FromSource element requires a \textit{sender} attribute. This should refer to the name of a AnalogSendPort or EventSendPort in the Cell{}\withelemComponent{}\withelemComponentClass of the source population. The transmission mode of the port (i.e. \lq\lq{}analog\rq\rq{} or \lq\lq{}event\rq\rq{}) should match that of the port referenced by the \textit{receiver} attribute.

\subsubsection{Receiver attribute}
Each FromSource element requires a \textit{receiver} attribute. This should refer to the name of a AnalogReceivePort, EventReceivePort or AnalogReducePort in the Component{}\withelemComponentClass in the enclosing \\
...
\subsubsection{Receiver attribute}
Each FromDestination element requires a \textit{receiver} attribute. This should refer to the name of a AnalogReceivePort, EventReceivePort or AnalogReducePort in the Component{}\withelemComponentClass in the enclosing \\
Source/Destination/Plasticity/Response element.  The transmission mode of the port (i.e. \lq\lq{}analog\rq\rq{} or \lq\lq{}event\rq\rq{}) should match that of the port referenced by the \textit{sender} attribute.
...
\subsubsection{Sender attribute}
Each FromPlasticity element requires a \textit{sender} attribute. This should refer to the name of a AnalogSendPort or EventSendPort in the Cell->Component{}\withelemComponentClass of the source population. The transmission mode of the port (i.e. \lq\lq{}analog\rq\rq{} or \lq\lq{}event\rq\rq{}) should match that of the port referenced by the \textit{receiver} attribute.

\subsubsection{Receiver attribute}
Each FromPlasticity element requires a \textit{receiver} attribute. This should refer to the name of a AnalogReceivePort, EventReceivePort or AnalogReducePort in the Component{}\withelemComponentClass in the enclosing Source/Destination/
...
\subsubsection{Receiver attribute}
Each FromResponse element requires a \textit{receiver} attribute. This should refer to the name of a AnalogReceivePort, EventReceivePort or AnalogReducePort in the Component{}\withelemComponentClass in the enclosing Source/Destination/\\
Plasticity/Response element.  The transmission mode of the port (i.e. \lq\lq{}analog\rq\rq{} or \lq\lq{}event\rq\rq{}) should match that of the port referenced by the \textit{sender} attribute.
...
In version 1.0, the Delay element specifies the delay between the pre-synaptic cell port and both the Plasticity\withelemComponent and Response\withelemComponent. In future versions, it is planned to include the delay directly into the port-connection objects (i.e. FromSource, FromDestination, etc...) to allow finer control of the delay between the different components.

\subsubsection{Units attribute}
The \textit{units} attribute specifies the units of the delay and should refer to the name of a Unit element in the document scope. The Unit{}\withelemDimension should be temporal, i.e. have $t=1$ and all other SI dimensions set to 0. 
...
The abstraction layer description of the IAF input neuron ComponentClass{}\withelemDynamics is:
\begin{lstlisting}[style=XML]
<?xml version='1.0' encoding='UTF-8'?>
<NineML xmlns="http://nineml.net/9ML/1.0"
	xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
...
and the description of the COBA ComponentClass{}\withelemDynamics is:
\begin{lstlisting}[style=XML]
<?xml version='1.0' encoding='UTF-8'?>
<NineML xmlns="http://nineml.net/9ML/1.0"
	xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
...
The cell Component{}\withelemDynamics are parameterized and connected together in the User Layer via Population, Selection and 
Projection elements:
