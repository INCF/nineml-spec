...
*/
  ...
  inline
  void RingBuffer::add_value(const long_t offs, const double_t v)
  {
    buffer_[get_index_(offs)] += v;
  }

  inline
  void RingBuffer::set_value(const long_t offs, const double_t v)
  {
    buffer_[get_index_(offs)] = v;
  }

  inline
  double RingBuffer::get_value(const long_t offs)
  {
  ...
  inline 
  size_t RingBuffer::get_index_(const delay d) const
  {
  ...
  inline
  void MultRBuffer::add_value(const long_t offs, const double_t v)
  {
    assert(0<= offs && (size_t)offs < buffer_.size());
    buffer_[get_index_(offs)] *= v;
  }
  ...
  inline
  double MultRBuffer::get_value(const long_t offs)
  {
    assert(0 <= offs && (size_t)offs < buffer_.size());
    assert((delay)offs < Scheduler::get_min_delay());
  ...
  inline 
  size_t MultRBuffer::get_index_(const delay d) const
  {
    const long_t idx = Scheduler::get_modulo(d);
