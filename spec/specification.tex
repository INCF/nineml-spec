...
simulator developers and developers of simulator-independent languages (NeuroML, PyNN) (see \ref{sec:task_force}).
...
NineML documents must be enclosed within \NineML XML tags.
...
Only seven document-level elements are allowed to reside directly within \NineML elements: \Component, \ComponentClass, \Unit, \Dimension, \Population, \Projection and \Selection. Each element should be uniquely identified by its \textit{name} attribute within the scope of the document (see \identifier). 

\Unit \notice and \Dimension elements must be defined within the document they are referenced, whereas the remaining elements can be referred to in other NineML documents (see \Reference and \Definition).

\subsubsection{Xmlns attribute}

The \textit{xmlns} attribute is required and should refer to the \URL corresponding to the correct NineML version, which for version 1.0 is \textit{http://nineml.net/9ML/1.0} (see \href{http://www.w3.org/TR/REC-xml-names/}{http://www.w3.org/TR/REC-xml-names/}).
...
Movements between dynamic regimes occurs via transitions, and have vanishing temporal extents (i.e. they are event-like). There are two types of transitions, condition-triggered transitions (see \OnCondition), which are evoked when an associated trigger expression becomes true, or event-triggered transitions (see \OnEvent), which are evoked when an associated event port receives an event from an external component. Multiple state assignments can be sent and multiple events can be sent within a single transition block.
...
guarantee about the value of V. It is left to the user to ensure such situations do not occur, and
...
\emph{Cascading} of events is permitted, which in theory 
could be recursive through components depending on their connectivity. 
\warning It is the users responsibility to ensure
that this does not occur with zero delay. Implementations may decide to terminate after 
a given number of recursive cascades (say 1000) to prevent infinite
...
derived from random distributions. \notice The \OnCondition block is triggered when the boolean result of the \Trigger statement changes from \emph{false} to \emph{true}.  
...
\item \RandomValue, a return value derived from a random distribution.
\end{itemize}
...
\subsubsection{Port attribute}
Each \RandomValue requires a \textit{port} attribute, which selects which port of the \Component to read.
