...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    *Distribution & singleton & yes\\
    \bottomrule
  \end{edtable}
\end{table}

\draftnote{I will finish adding the required attributes (copying the names used in UncertML) for these distributions later this week}

\subsection{NormalDistribution}
\label{sec:NormalDistribution}

\subsection{LogNormalDistribution}
\label{sec:LogNormalDistribution}

\subsection{GammaDistribution}
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    probability & \Parameter{}@name & yes\\
    \bottomrule
  \end{edtable}
\end{table}

All cells in the source population are connected to cells in the destination population with a probability defined by the \textit{probability} attribute.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    source-indices & \Parameter{}@name & yes\\
    target-indices & \Parameter{}@name & yes\\
    \bottomrule
  \end{edtable}
\end{table}
...
Each \ExplicitConnectionList element requires a \textit{source-indices} attribute. This attribute should refer to a name of a dimensionless \Parameter in the enclosing \ComponentClass. The properties supplied to this parameter should be a \ArrayValue or \ExternalArrayValue drawn from the set $\{1,\ldots,M\}$ where $M$ is the size of the source population and be the same length as the property supplied to the \textit{target-indices} parameter.

\subsubsection{Target-indices attribute}

Each \ExplicitConnectionList element requires a \textit{target-indices} attribute. This attribute should refer to a name of a dimensionless \Parameter in the enclosing \ComponentClass. The properties supplied to this parameter should be a \ArrayValue or \ExternalArrayValue drawn from the set $\{1,\ldots,N\}$ where $N$ is the size of the source population and be the same length as the property supplied to the \textit{source-indices} parameter.
