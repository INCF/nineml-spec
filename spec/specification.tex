...
\newcommand{\PruneConnections}{\defRef{\textbf{\class{PruneConnections}}\xspace}{sec:PruneConnections}}
\newcommand{\TargetNumber}{\defRef{\textbf{\class{TargetNumber}}\xspace}{sec:TargetNumber}}
\newcommand{\OrderOfPreference}{\defRef{\textbf{\class{OrderOfPreference}}\xspace}{sec:OrderOfPreference}}
...
\newcommand{\FromComponent}{\defRef{\textbf{\class{FromComponent}}\xspace}{sec:FromComponent}}
...
\ConnectCondition elements specify the conditions under which \incomplete

\subsection{PruneConnections}
\label{sec:PruneConnections}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{PruneConnections Structure}}}\\
    \toprule
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \Number & singleton & yes \\
    \OrderOfPreference & singleton & no\\
    \bottomrule
  \end{edtable}
\end{table}
...
\subsection{OrderOfPreference}
\label{sec:OrderOfPreference}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{OrderOfPreference Structure}}}\\
    \toprule
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \MathInline & singleton & yes \\     
...
or derived from a \Component via a \FromComponent element.
...
\subsection{FromComponent}
\label{sec:FromComponent}
...
The array of values represented by a \FromComponent element is populated from the output port of a component. The size of the generated array is determined by the size of the container (i.e. \Population or \Projection) the \FromComponent is nested within.

\subsubsection{Port attribute}
Each \FromComponent requires a \textit{port} attribute, which selects which port of the \Component to read.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Text format} & \; & \em{Required} \\
    \midrule
    \primtype{non-negative integer} & \; & yes\\
    \bottomrule
  \end{edtable}
\end{table}
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Element type} & Multiplicity & \em{Required} \\
    \midrule \relax
    \SingleValue | \ArrayValue | \ExternalArrayValue | \FromComponent & singleton & yes\\
    \bottomrule
  \end{edtable}
\end{table}
