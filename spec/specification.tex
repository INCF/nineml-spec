...
The mapping array is a list of indices corresponding to the \emph{mapID}'s of the domains. This layout is analogous to "type" column in the SWC format. In fact it would be nearly be possible to use an SWC file as an ExternalArrayValue file, with the exception that it would need column headers and the root node would need to be omitted. This could also be substituted for a component that algorithmically maps the segments to different domains.
