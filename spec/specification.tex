...
\newcommand{\MathML}{\href{http://www.w3.org/Math/}{MathML}\xspace}
...
\newcommand{\TargetNumber}{\defRef{\textbf{\class{TargetNumber}}\xspace}{sec:TargetNumber}}
...
\item false
\end{itemize}
where $t$ is the elapsed simulation time within a \Dynamics block and `true' and 'false' are the logical identities. 
...
\RandomVariable elements represent randomly varying quantities, which are drawn from standard families of random distributions. The random distributions are defined via \href{http://www.uncertml.org/distributions}{UncertML (http://www.uncertml.org/distributions)}, with the caveats that the "multi-values" are not permitted and numerical arguments passed to UncertML distributions can be substituted for the names of dimensionless properties and aliases of the enclosing \ComponentClass.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \SelectConnections & singleton & no\\
    \Alias & set & no\\
    \Constant & set & no\\
    \RandomVariable & set & no\\
    \Piecewise & set & no\\    
...
\ConnectionRule elements are the top-level container for \ComponentClass elements that define network connectivity. Typically, values are read from \AdditionalProperty elements within the source and destination population via \PropertyReceivePort{}s .
...
\ConnectCondition elements specify the conditions under which a source and destination cell should be connected. If the \MathInline expression in it evaluates to true then the cells are provisionally connected.
...
\draftnote{Could alternatively be called Static to contrast with Dynamics elements or maybe Relationship}
...
or derived from a \Component via a \FromFunction element.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \Component | \Reference{}\pointto\Component & singleton & yes \\
    \bottomrule
  \end{edtable}
\end{table}

The array of values represented by a \FromFunction element is populated from the output port of a component. The size of the generated array is determined by the size of the container (i.e. \Population or \Projection) the \FromFunction is nested within.
...
The number of cells in the population is specified either by a non-negative integer provided in the text of the \Number element or from an \IndexSendPort within a \Function component.

\subsubsection{Text format}
The text of the \Number element should contain an \primtype{integer} representing the size of the population if and only if it isn't derived from a \FromFunction\withelem\IndexSendPort port. 
...
\subsection{FromProperty}
\label{sec:FromProperty}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{FromProperty Structure}}}\\
    \toprule
     \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
