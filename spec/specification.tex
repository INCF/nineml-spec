...
\newcommand{\ConnectIf}{\defRef{\textbf{\class{ConnectIf}}\xspace}{sec:ConnectIf}}
\newcommand{\Prune}{\defRef{\textbf{\class{Prune}}\xspace}{sec:Prune}}
...
\newcommand{\ContinuousExpression}{\defRef{\textbf{\class{ContinuousExpression}}\xspace}{sec:ContinuousExpression}}
...
\newcommand{\ExtraProperty}{\defRef{\textbf{\class{ExtraProperty}}\xspace}{sec:ExtraProperty}}
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \ContinuousExpression & singleton & yes \\
...
\subsection{ContinuousExpression}
\label{sec:ContinuousExpression}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{ContinuousExpression Structure}}}\\
    \toprule
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
...
Network connectivity is defined via extensible components to promote reuse of connection logic. Connections are provisionally connected if the \ConnectIf evaluates to true for a given connection, and then can be pruned to a fixed number (which can be derived from a random value) if a \Prune element is provided.
...
\ConnectionRule elements are the top-level container for \ComponentClass elements that define network connectivity. Typically, values are read from \ExtraProperty elements within the source and destination population via \ValueReceivePort{}s .

\subsection{ConnectIf}
\label{sec:ConnectIf}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{ConnectIf Structure}}}\\
    \toprule
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \MathInline & singleton & yes \\
...
\ConnectIf elements specify the conditions under which 
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{Prune Structure}}}\\
    \toprule
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \TargetNumber & singleton & yes \\
...
\subsection{TargetNumber}
\label{sec:TargetNumber}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{TargetNumber Structure}}}\\
    \toprule
    \em{Text format} & \; & \em{Required} \\
    \midrule
...
\subsection{ExtraProperty}
\label{sec:ExtraProperty}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{ExtraProperty Structure}}}\\
    \toprule
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    name & \identifier & yes\\
...
\ExtraProperty elements are used to append properties to the cells in population that are used by connectivity rules.

\subsubsection{Name attribute}
Each \ExtraProperty requires a \textit{name} attribute. This should refer to the name of a \Parameter in the corresponding \ComponentClass of the \Component.

\subsubsection{Units attribute}
Each \ExtraProperty element requires a \emph{units} attribute. The \textit{units} attribute specifies the units of the quantity and should refer to the name of a \Unit element in the global scope. For a dimensionless units a \Unit\withelem\Dimension with no SI dimensions can be used. The SI dimensions of the \Unit\withelem\Dimension should match the SI dimensions of the corresponding \Parameter\withelem\Dimension.
...
Sets of \Projection elements are collected into networks.

\note{The \Population{}s that are referenced in the \Projection{}s are implicitly included).}
...
\subsection{MultiCompartmentComponent}
\label{sec:MultiCompartmentComponent}
...
\subsubsection{Name attribute}
Each \Domain requires a \textit{name} attribute, which should be a valid \identifier and uniquely identify the \Domain in the \MultiCompartmentComponent scope.

\subsubsection{MapID attribute}
Each \Domain requires a \text{mapID} attribute, which should be a non-negative integer and unique amongst the \Domain element in the enclosing \MultiCompartmentComponent. The \textit{mapID} is used to map the compartments onto the domain.
...
\MultiPortExposure elements expose ports within \Domain elements outside the \MultiCompartmentComponent. They are distinguished from \PortExposure elements because when the \MultiCompartmentComponent is flattened multiple ports are created, one for each compartment of the specified \Domain, or all compartments if no \Domain is specified.
...
Each \MultiPortExposure requires a \textit{domain} attribute, which should refer to a \Domain within the \MultiCompartmentComponent scope.
...
\subsubsection{Domain attribute}
Each \FromProximal can define a \textit{domain} attribute. If present the \textit{domain} attribute should refer to a \Domain within the \MultiCompartmentComponent scope.
