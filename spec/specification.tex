...
\Piecewise elements are used to describe piecewise mathematical functions. The syntax is inspired by the \MathML format for piecewise functions. Each \Piece element describes a continuous segment of the function that is valid over the range its condition (see \Condition) evaluates to true. It is critical that the combination of parameters and states under which the \Piece\withelem\Condition blocks in a single \Piecewise element evaluate to true are exclusive. As in \MathML, the onus is on the model designer to ensure that the conditions blocks do not overlap. For combinations of parameters and states that are not covered by any \Piece\withelem{}\Condition block the \Piecewise function evaluates to the value of the \Otherwise\withelem{}\MathInline expression.
...
The \Otherwise block is active within a \Piecewise expression whenever all other \Piece\withelem{}\Condition blocks evaluate to false.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \midrule
    name & \identifier & yes\\
    conversion & `round' | `ceil' | `floor' & yes \\
    \bottomrule
  \end{edtable}
\end{table}
...
\subsubsection{Conversion attribute}
Each \IndexSendPort requires a \textit{conversion} attribute, which specifies the conversion from floating point to integer numbers.
