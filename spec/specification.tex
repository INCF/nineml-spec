...
``Extensions'' increase the descriptive power of NineML by providing compact, high-level descriptions of models that can also be expressed in ``Core'' NineML (i.e. syntactic sugar). Envisaged examples include more concise forms for kinetic equations and multi-component/compartment dynamic models. 
...
\begin{itemize}
\item Models (or part thereof) written in the Extension format must be collapsible to Core NineML without change in behaviour.
...
\item Stand-alone tools that perform the two-way conversion between the extended and core forms (preferably written in commonly used languages such as XSLT, Python, Java, etc...) must be downloadable from a publicly available \URL (a section of the NineML website will be available for this purpose).
\item Non-standard extensions must be defined within a unique namespace
\item Detailed specifications (preferably in the same format as this document) must be made publicly available along with the conversion tools.
...
%\StandardLibrary require a \textit{url} attribute, which specifies a \URL that describes the behaviour of the standard library member.
%
%\subsubsection{Text}
...
\draftnote{I have removed the \Quantity{s} in favour of adding units to the \Property elements. I will create a GitHub issue about this where we can discuss this and potentially decide to change it back if you like}
\subsection{Property}
\label{sec:Property}
...
The \textit{units} attribute specifies the units of the quantity and should refer to the name of a \Unit element in the global scope. If it is omitted then the quantity is considered to be dimensionless. The \Dimension of the \Unit should have matching power attributes (i.e. `\emph{l}', `\emph{m}', `\emph{k}', `\emph{t}', `\emph{n}', `\emph{i}' and `\emph{j}') to the \Dimension of the corresponding \Parameter.
...
\subsubsection{Url attribute}
The \textit{url} attribute specifies a \URL for the external data file.
