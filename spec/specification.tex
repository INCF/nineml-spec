...
\newcommand{\ComponentClassNL}{\textbf{\textcolor{ninemlblue}{\class{ComponentClass}}}\xspace} % Component class reference style without an associated link to the section
...
The main building block of the Abstraction Layer is the \ComponentClassNL.
The \ComponentClassNL is intended to package together a collection of
...
model, such as neuron model for instance, belong in a single \ComponentClassNL element.
A \ComponentClassNL element can be used to represent either a specific model of a neuron or a
...
The interface is the \emph{external} view of the \ComponentClassNL element that defines
what inputs and outputs the component exposes to other {\ComponentClassNL}es and the
parameters that can be set for the \ComponentClassNL. The interface consists of
instances of ports and \Parameter.

As well as being able to specify the communication of continuous values,
{\ComponentClassNL} elements are also able to specify the emission and the reception
...
Ports allow instantiations of {\ComponentClassNL} elements to communicate between each
...
A \ComponentClassNL is composed of:
\begin{itemize}
\item {\Parameter}s for the \ComponentClassNL
...
dynamic of the model while the Interface block allows the \ComponentClassNL to
communicate with other instantiations of the \ComponentClassNL and with the User
Layer.
\end{itemize}
...
Each \ComponentClassNL requires a \textit{name} attribute, which should be a valid \identifier and uniquely identify the {\ComponentClassNL} in the current scope.
...
\subsubsection{Name attribute}
Each \Parameter requires a \textit{name} attribute, which is a valid \identifier and uniquely identify the \Parameter from all other elements in the \ComponentClass.
...
\note{This format for \Dimension is the same as is used in 
...
right-hand-side for  {\Alias}es, {\TimeDerivative}s \& {\StateAssignment}s). All numbers/variables in MathInline expressions are assumed to be real valued numbers.
...
where $pi$ is the mathematical constant $\pi$ and $t$ is the elapsed simulation time when referenced within \Dynamics blocks.
