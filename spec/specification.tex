...
\item Either a \Dynamics, \ConnectionRule or \RandomDistribution block. \Dynamics blocks represent the internal dynamics of the model. \ConnectionRule blocks specify the rule by which populations are connected in projections. \RandomDistribution block defines random distribution.
\end{itemize}
...
only supported operations is \lq\lq{}$+$\rq\rq{}, which sums all the incoming port values.
...
An \EventSendPort specifies a channel over which events can be transmitted from the component. 
...
An \EventReceivePort specifies a channel over which events can be received by the component. 
...
these regimes, and the state variables will evolve accordingly (see \ref{SimpleRegimeGraph}).
...
\emph{\textbf{ProbabilisticConnectivity}}
...
\emph{\textbf{ExplicitConnectionList}}
...
\emph{\textbf{RandomFanIn}}

\emph{\textbf{RandomFanOut}}
