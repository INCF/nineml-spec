...
Layer are clearly defined (each Child elements belongs to either one).
...
NineML documents must be enclosed within an \NineML element.
...
The \notice use of \Constant elements to hold fixed model parameters is \emph{highly discouraged} since this breaks the division of semantic layers (abstraction and user), which is a key feature of NineML (see \ref{sec:scope}).
...
\SingleValue and \RandomValue elements used in properties of a projection (in the \Connectivity, \Response, \Plasticity and \Delay elements) take the size of the number of connections made. Explicitly array values, \ArrayValue and \ExternalArrayValue, are only permitted with \Connectivity rules where the number of connections is predetermined (i.e. \emph{OneToOne}, \emph{AllToAll} and \emph{ExplicitConnectionList}) and must be the same size as the number of connections made. \notice Explicit arrays are ordered by the indices
\begin{equation}
i_{\mathrm{value}} = i_{\mathrm{source}} * N_{\mathrm{dest}} + i_{\mathrm{dest}}
\end{equation}
where $i_{\mathrm{value}}$, $i_{\mathrm{source}}$ and $i_{\mathrm{dest}}$ are the indices of the array entry, and the source and destination cells respectively, and $N_{\mathrm{dest}}$ is the size of the destination population. Indices that do not correspond to connected pairs are omitted.
