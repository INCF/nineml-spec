...
The \FromComponent element specifies a port connection to the containing \SubComponent component from another \SubComponent. 
...
\MultiPortExposure elements expose ports within \Domain elements outside the \MultiCompartmentComponent. They are distinguished from \PortExposure elements because when the \MultiCompartmentComponent is flattened multiple ports are created, one for each compartment of the specified \Domain.
...
\begin{lstlisting}
  <?xml version="1.0" encoding="UTF-8"?>
  <NineML xmlns="http://nineml.net/9ML/1.0">
    <MultiCompartmentClass name="ExampleMultiCompartmentClass">
