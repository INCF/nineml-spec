...
\newcommand{\FromSubComponent}{\defRef{\textbf{\class{FromSubComponent}}\xspace}{sec:FromSubComponent}}
...
\SubComponent elements can refer to any \ComponentClass\withelem\Dynamics class. They elements are explicitly connected to other \SubComponent elements via ports specified by \ReceiveConnection\withelem{}\FromSubComponent elements. Apart from accepting \ReceiveConnection elements, \SubComponent elements are the defined the same as \Component elements.
...
\subsection{FromSubComponent}
\label{sec:FromSubComponent}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{FromSubComponent Structure}}}\\
    \toprule
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
...
The \FromSubComponent element specifies a port connection to the enclosing \SubComponent component from another \SubComponent. 

\subsubsection{Component attribute}
Each \FromSubComponent requires a \textit{component} attribute, which should refer to a \SubComponent within the \MultiComponent scope.

\subsubsection{Port attribute}
Each \FromSubComponent requires a \textit{port} attribute, which should refer to the name of a port in the corresponding \SubComponent.
