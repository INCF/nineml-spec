...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \Dynamics | \ConnectionRule | \Function & singleton & yes \\
    \bottomrule
  \end{edtable}
\end{table}
...
\item \Function, the component class defines random distribution.
\end{itemize}}
\end{itemize}
...
\PropertySendPort{}s are used to return quantities from \Component\withelem\Function{}s to populate \DerivedProperty elements.
...
Functions are used to define relationships between parameters over members of a container.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{Function Structure}}}\\
    \toprule
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \Alias & set & no\\
...
\subsection{FromFunction}
\label{sec:FromFunction}
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \Component{}\withelem\Function | \Reference{}\pointto\Component{}\withelem\Function & singleton & yes \\
    \bottomrule
  \end{edtable}
\end{table}

The values of \DerivedProperty elements are populated from \PropertySendPort{}s of \Component{}\withelem\Function{}s, as specified by \FromFunction elements. The function is evaluated for each item in the container (i.e. \Population, \Projection or \MultiCompartmental) it is enclosed by.

\subsubsection{Port attribute}
Each \FromFunction requires a \textit{port} attribute, which selects which port of the \Component to read.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \Property & set & no \\
    \DerivedProperty & set & no \\
    \Component{}\withelem{}\Function & set & no\\
    \bottomrule
  \end{edtable}
\end{table}
...
The number of cells in the population is specified either by a non-negative integer provided in the text of the \Size element or from an \IndexSendPort within a \Function component.

\subsubsection{Text format}
The text of the \Size element should contain an \primtype{integer} representing the size of the population if and only if it isn't derived from a \FromFunction{}\withelem\IndexSendPort port. 
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Element type} & Multiplicity & \em{Required} \\
    \midrule \relax
    \SingleValue | \ArrayValue | \ExternalArrayValue | \FromFunction & singleton & yes\\
    \bottomrule
  \end{edtable}
\end{table}
