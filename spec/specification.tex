...
Distributions are used to define relationships between parameters over members of a container.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \toprule
     \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    of & `source' | `destination' | `connection' | `sourceDomain' | 'destinationDomain' & yes\\
    \bottomrule
  \end{edtable}
\end{table}

\subsubsection{Of attribute}
The \textit{of} attribute specifies the index and should refer to the name of a \Unit element in the global scope. The \Unit{}\withelem\Dimension should be temporal, i.e. have $t=1$ and all other SI dimensions set to 0. 
