...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \MathInline | \Piecewise & singleton & yes \\ 
    \bottomrule
  \end{edtable}
\end{table}
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \midrule
    name & \identifier & yes\\
    distribution & \textit{uncertml-distribution}@name & yes\\    
    units & \Unit{}@name & yes\\
    \midrule
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
...
\draftnote{Realised that this can't be a component if we are going to separate the Abstraction and User Layers (i.e. It would require using a \Component within a \ComponentClass) so I have separated it out. Also random distributions derived from standard distributions (the only ones I can see people wanting to manually define), can be specified using \Function elements and passed to \Component\withelem{}\Property elements}

The names and arguments of the available random distributions match the UncertML definitions that can be found at \href{http://www.uncertml.org/distributions}{http://www.uncertml.org/distributions}. 

\subsubsection{Name attribute}
Each \Constant requires a \textit{name} attribute, which should be a valid \identifier and uniquely identify the {\Dimension} in current the scope.

\subsubsection{Distribution attribute}
The \textit{distribution} attribute is required and should point to a \URL in the \href{http://www.uncertml.org/distributions/}{http://www.uncertml.org/distributions/} directory. The subset of the UncertML distributions that should be implemented are by NineML compliant packages are,
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{Argument Structure}}}\\
    \toprule
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    name & \textit{uncertml-argument}@name & yes\\
    \midrule
    \em{Text format} & \; & \em{Required} \\
    \midrule
    \primtype{real number} & \; & yes \\    
