...
Component classes that contain \Dynamics elements define dynamic models such as neurons, post-synaptic responses or the plasticity of synaptic weights. In these components, state variables are evolved by sets of ordinary differential equations (ODE) to describe the behaviour of the model. The state of the model can transition between distinct regimes with different sets of differential equations via `transitions'. \ref{fig:simple_regime_graph} illustrates a hypothetical transition graph between different ODE regimes, which could be used to separately model sub-threshold and refractory periods for example.
...
\end{equation}
which can contain of references to any combination of \StateVariable, \Parameter, \AnalogReceivePort, \AnalogReducePort and \Alias elements with the exception of those derived from \RandomDistribution components.
...
Transitions define the transition between \Regime blocks.
...
Movement between several instances of \Regime happens via a transition.
...
in \ref{SimpleRegimeGraph}). There are two types of transitions:
...
Transitions have a vanishing temporal extent (i.e. they are
...
In future versions, built-in connectivity rules are to be replaced with arbitrary connection rule functions.
