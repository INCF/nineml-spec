...
%% First authors: Ivan Raikov <raikov@oist.jp>, Thomas G. Close <tclose@oist.jp>
...
%%
%% Copyright (C) 2014 Okinawa Institute of Science and Technology Graduate University
%%
%% ============================================================================
...
\newcommand{\Set}{\defRef{\textbf{\class{Set}}\xspace}{sec:Set}}
...
\section{Unit and Dimensions in NineML}
...
The \textit{m} attribute specifies the power of the mass dimension in the \Dimension. If omitted it the power is zero.

\subsubsection{L attribute}
The \textit{l} attribute specifies the power of the length dimension in the \Dimension. If omitted it the power is zero.

\subsubsection{T attribute}
The \textit{t} attribute specifies the power of the time dimension in the \Dimension. If omitted it the power is zero.

\subsubsection{I attribute}
The \textit{i} attribute specifies the power of the current dimension in the \Dimension. If omitted it the power is zero.

\subsubsection{N attribute}
The \textit{n} attribute specifies the power of the amount-of-substance dimension in the \Dimension. If omitted it the power is zero.

\subsubsection{K attribute}
The \textit{k} attribute specifies the power of the temperature dimension in the \Dimension. If omitted it the power is zero.

\subsubsection{J attribute}
The \textit{j} attribute specifies the power of the luminous-intensity dimension in the \Dimension. If omitted it the power is zero.
...
\subsubsection{Dimension attribute}
Each \AnalogReducePort requires a \textit{dimension} attribute. This attribute specifies the dimension of the units of the quantity that is expected to be passed to the \AnalogReducePort and should refer to the name of a \Dimension element in the global scope.
...
\draftnote{Are {\Regime}s required? Could you have a component that simply consists of aliases?}
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    port & [\EventReceivePort | \EventReducePort{}]@name & yes\\
    \midrule
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
...
C89. All
numbers/variables are assumed to be real valued numbers.
...
\draftnote{writing this up I thought it was a bit weird that the parameters in the component interface were \lq\lq{}magically\rq\rq{} connected up with the right parameters in the built-in connection rule. Therefore I have introduced a new element for each built-in type, which have attributes that will explicitly link to the parameter names. Since these will only be defined once I didn\rq{}t see the harm in this.}
...
\draftnote{Looking forward to version 2 I was thinking that this connectivity rule would make a nice target for many \lq\lq{}extended\rq\rq{} connection rule schemes. I don\rq{}t quite like the format of the value list I have come up with though, so if you would prefer to take it out and introduce something else in Version 2 that is okay.}
...
\draftnote{I thought that we should probably be specific for the built-in random distributions that are required in 9ML implementations. Going through \href{http://www.uncertml.org/}{http://www.uncertml.org/} I have picked out the ones that I suspect will be most useful to start off with. Are there any others you think I should include (or just all of them)?}
...
\draftnote{I have moved the \Component{}\withelem\RandomDistribution outside of the a \lq\lq{}RandomDistribution\rq\rq{}
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \SingleValue | \Component{}\withelem\RandomDistribution | \ArrayValue | \ExternalArrayValue & singleton & yes \\
    \bottomrule
  \end{edtable}
\end{table}
...
\draftnote{Instead of \lq\lq{}SingleValue\rq\rq{}, I thought \SingleValue was a better name for this, thoughts?}
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
...
\draftnote{I thought that \ArrayValue and \ArrayValueRow would be sufficient to provide the XML version explicit lists for demonstration purposes (see GitHub \#21)}
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    column-name & Data column name in external file & no\\    
    \bottomrule
  \end{edtable}
\end{table}
...
\draftnote{Again, should we require column names to be provided to avoid problems if a second columns is added later?}
...
\draftnote{Needed to change since we are using \Prototype instead of \lq\lq{}Reference\rq\rq{} to refer to previously parameterised {\Component}s. I first thought of CellType but then I thought using \lq\lq{}Type\rq\rq{} might be confusing with the \lq\lq{}Class\rq\rq{}es in the Abstraction Layer.}
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \Reference{}\pointto\Population | \Reference{}\pointto\Set & singleton & yes \\
...
\draftnote{I have permitted all the From* to be accessed from each of the four components of a synapse (source, dest, response, plasticity). I am not sure if this is a good idea or whether we should only allow the destination to access the post-synaptic response for example. It could make it unnecessarily complex to implement in some simulators...}
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    receiver & [\AnalogReceivePort | \EventReceivePort | \AnalogReducePort | \EventReducePort{}]@name & yes\\
    \bottomrule
  \end{edtable}
\end{table}
...
Each \FromSource element requires a \textit{receiver} attribute. This should refer to the name of a \AnalogReceivePort, \EventReceivePort, \AnalogReducePort or \EventReducePort in the \Component{}\withelem\ComponentClass in the enclosing \Source/\Destination/\Plasticity/\Response element.  The transmission mode of the port (i.e. \lq\lq{}analog\rq\rq{} or \lq\lq{}digital\rq\rq{}) should match that of the port referenced by the \textit{sender} attribute.
...
\draftnote{Set doesn\rq{}t seem to be the right name considering we are trying to conserve cell indices (see GitHub \#34)}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{Set Structure}}}\\
    \toprule
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Text format} & \; & \em{Required} \\
    \midrule \relax
    [\Population | \Set{}]@name & \; & yes\\
    \bottomrule
  \end{edtable}
\end{table}

\subsubsection{Index attribute}
Each \Item requires a \textit{index} attribute. This attribute specifies the order in which the {\Population}s in the \Set are concatenated and thereby the indices of the cells within the combined \Set. Each \textit{index} must be non-negative, unique amongst the set of \Item{}@index enclosed in the \Concatenate element, and the set of indices must be contiguous for a single \Concatenate element.
