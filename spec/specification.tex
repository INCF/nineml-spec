...
\begin{itemize}
\item \StateVariable (section~\ref{sec:StateVariable})
\item \Regime (section~\ref{sec:Regime}) that describe the dynamics of the
system for a particular domain of \StateVariable
\item \Transition (section~\ref{sec:Transition}) that links two
...
\item \Alias (section~\ref{sec:Alias})
\end{itemize}
...
\subsubsection{Transition-to attribute}
\OnEvent can have a \textit{transition-to} attribute, which should refer to the name of a \Regime element in the \ComponentClass that the dynamics block will transition to when the \Trigger condition is met.
