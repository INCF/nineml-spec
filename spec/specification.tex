...
\draftnote{I can\rq{}t find the luminous intensity abbreviation in the NeuroML specs}
The \textit{?} attribute specifies the power of the luminous-intensity dimension in the \Dimension. If omitted it the power is zero.
...
\subsubsection{PowTen attribute}
Each \Unit requires a \textit{powTen} attribute. This attribute specifies the relative scale of the units compared to the equivalent SI units in powers of ten.
...
\draftnote{Is there a point of having an \EventReducePort, could multiple send ports just be connected to any \EventReceivePort?}
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \HomogeneousValue | \Component{}\withelem\RandomDistribution | \ValueList | \ExternalValueList & singleton & yes \\
    \bottomrule
  \end{edtable}
\end{table}
...
\draftnote{I thought that \ValueList and \ValueListRow would be sufficient to provide the XML version explicit lists for demonstration purposes (see GitHub \#21)}
...
The \textit{index} attribute specifies the index of the \ValueListRow in the \ValueList. It must be non-negative, unique amongst the set of \ValueListRow{}@index in the list, and the set of indices must be contiguous for a single \ValueList.
