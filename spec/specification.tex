...
\newcommand{\ReceivesConnection}{\defRef{\textbf{\class{ReceivesConnection}}\xspace}{sec:ReceivesConnection}}
...
\subsection{ReceivesConnection}
\label{sec:ReceivesConnection}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{ReceivesConnection Structure}}}\\
    \toprule
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule \relax
...
The \ReceivesConnection element specifies a receive port which will receive a connection from a port of a remote component. Depending on the context in which \ReceivesConnection is placed, the role of the components it can receive from vary. Therefore, it accepts a range of elements (all named \emph{From*}), which specify the relationship of the remote component to the enclosing element (e.g. \FromSource, \FromDestination and \FromSynapse  in \Projection containers).

\subsubsection{Port attribute}
Each \ReceivesConnection element requires a \textit{port} attribute. This should refer to the name of a \AnalogReceivePort, \EventReceivePort or \AnalogReducePort in the \ComponentClass associated with the enclosing element.
...
\SubComponent elements can refer to any \ComponentClass\withelem\Dynamics class. They elements are explicitly connected to other \SubComponent elements via ports specified by \ReceivesConnection\withelem{}\FromComponent elements. Apart from accepting \ReceivesConnection elements, \SubComponent elements are the defined the same as \Component elements.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{FromComponent Structure}}}\\
    \toprule
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
...
The \FromComponent element specifies a port connection to the enclosing \SubComponent component from another \SubComponent. 

\subsubsection{Component attribute}
Each \FromComponent requires a \textit{component} attribute, which should refer to a \SubComponent within the \MultiComponent scope.

\subsubsection{Port attribute}
Each \FromComponent requires a \textit{port} attribute, which should refer to the name of a port in the corresponding \SubComponent.
...
To handle the projections onto dendritic trees with multiple compartments an extension of the \Projection element must be used if either the source or destination populations consist of multi-compartmental models. It receives a \CompartmentConnectivity element which determines which compartments projections are made. 

In flattening the \MultiCompartmentProjection elements to Core NineML, a single \MultiCompartmentProjection is split into a set of \Projection elements, one for each possible source/destination compartment pairing.
