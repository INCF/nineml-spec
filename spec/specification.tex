...
\draftnote{Andrew, I was thinking after your suggestion to have a section describing the general purpose of ports that it would be good to move the top sections up a level and maybe have the \lq\lq{}Abstraction Layer\rq\rq{} and \lq\lq{}User Layer\rq\rq{} as latex \lq\lq{}parts\rq\rq{} and then introduce a new layer of sections for Ports, Dynamics, ConnectionRule, RandomDistribution, etc. What do you think?}
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    webpage & \href{http://en.wikipedia.org/wiki/Uniform_resource_locator}{URL} & yes\\
    \midrule
...
\subsubsection{Webpage attribute}
\StandardLibrary require a \textit{webpage} attribute, which specifies a \href{http://en.wikipedia.org/wiki/Uniform_resource_locator}{Uniform Resource Locator (URL)} that describes the behaviour of the standard library member.
...
\subsubsection{Transition-to attribute}
\OnEvent can have a \textit{transition-to} attribute, which should refer to the name of a \Regime element in the \ComponentClass that the dynamics block will transition to when the \OnEvent block is triggered by an incoming event.
...
An {\OnCondition} which persistently evaluates to True violates
...
\subsubsection{Transition-to attribute}
An \OnEvent can have a \textit{transition-to} attribute, which should refer to the name of a \Regime element in the \ComponentClass that the dynamics block will transition to when the \Trigger condition is met.
