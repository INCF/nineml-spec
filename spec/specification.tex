...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \Component{}\withelem\ConnectionRule | \Reference{}\pointto{}\Component\withelem\ConnectionRule& singleton & yes \\
    \ReceiveConnection & set & yes\\ 
    \bottomrule
  \end{edtable}
\end{table}
...
The \ReceiveConnection element specifies a receive port which will receive a connection from a port of a remote component. Depending on the context in which \ReceiveConnection is placed, the role of the components it can receive from vary. Therefore, it accepts a range of elements (all named \emph{From*}), which specify the relationship of the remote component to the enclosing element (e.g. \FromSource, \FromDestination and \FromSynapse  in \Projection containers).
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \toprule
     \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    port & [\AnalogSendPort | \EventSendPort{} | \Parameter{}@name]@name & yes\\
    \midrule
    \em{Element type} & Multiplicity & \em{Required} \\
    \Delay & singleton & no\\
    \bottomrule
