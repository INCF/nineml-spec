...
\newcommand{\Distribution}{\defRef{\textbf{\class{Distribution}}\xspace}{sec:Distribution}}
...
\newcommand{\FromDistribution}{\defRef{\textbf{\class{FromDistribution}}\xspace}{sec:FromDistribution}}
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \PropertySendPort & set & no\\    
    \PropertyReceivePort & set & no\\
    \IndexSendPort & set & no\\
    \IndexReceivePort & set & no\\
    \Dynamics | \ConnectionRule | \Distribution & singleton & yes \\
    \bottomrule
  \end{edtable}
\end{table}
...
\begin{itemize} 
\item \Dynamics, the component class defines a dynamic element such as neuron or post-synaptic response.
\item \ConnectionRule, the component class defines a rule by which populations are connected in projections.
\item \Distribution, the component class defines random distribution.
\end{itemize}}
\end{itemize}
...
As of NineML version 2.0, only inline mathematical expressions, which have similar syntax to the ANSI C89 standard, are supported. In future versions, it is envisaged that inline expressions will be either augmented or replaced with \MathML expressions.
...
\end{itemize}
\end{multicols}
All built-in functions are defined as per ANSI C89 with the exception of `\verb|round|', which has the same function as `\verb|nearbyint|' in the C89 standard, with `round-to-nearest' set in the floating point parameters \\(see \href{http://www.gnu.org/software/libc/manual/html\_node/Rounding-Distributions.html}{http://www.gnu.org/software/libc/manual/html\_node/Rounding-Distributions.html}).

The following symbols refer to built-in values,
\begin{itemize}
\item t
\item true
\item false
\end{itemize}
where $t$ is the elapsed simulation time within a \Dynamics block and `true' and 'false' are the logical identities. The true and false logical identities can also be represented by numerical values of $1$ and $0$, respectively.
...
\label{sec:RandomVariable}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{RandomVariable Structure}}}\\
    \toprule
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    name & \identifier & yes\\
    ...
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
   \href{http://www.uncertml.org/distributions}{UncertMLDistribution} & singleton & yes\\  
    \bottomrule
  \end{edtable}
\end{table}

\draftnote{I realised that RandomDistributions can't be components if we are going to separate the Abstraction and User Layers (i.e. It would require using a \Component within a \ComponentClass) so I have separated it out. Also random distributions derived from standard distributions (the only ones I can see people wanting to manually define), can be specified using \Distribution elements in this scheme.
see \href{https://github.com/INCF/nineml/issues/72}{GitHub Issue \#31}}

\RandomVariable elements represent randomly varying quantities, which are drawn from standard families of random distributions. The random distributions are defined via \href{http://www.uncertml.org/distributions}{UncertML (http://www.uncertml.org/distributions)}, with the caveats that the "multi-values" are not permitted and the numerical arguments passed to UncertML distributions can be substituted for the names of dimensionless properties and aliases of the enclosing \ComponentClass.

The subset of UncertML distributions available in NineML is
...
\note{Open source \textit{C} implementations of these distributions are available in the GNU Scientific Library, \href{http://www.gnu.org/software/gsl/}{http://www.gnu.org/software/gsl/}.}
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{Piecewise Structure}}}\\
    \toprule
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    name & \identifier & yes\\
    ...
    \midrule
    \Piece & set & yes \\
    \Otherwise & singleton & yes \\
    \bottomrule
  \end{edtable}
\end{table}

\draftnote{see \href{https://github.com/INCF/nineml/issues/74}{GitHub Issue \#74}}
\Piecewise elements are used to describe piecewise mathematical functions. The syntax is inspired by the \MathML format for piecewise functions. Each \Piece element describes a continuous segment of the function that is valid over the range its condition evaluates to true (see \Condition). It is critical that the combination of parameters and states under which the \Piece\withelem\Condition blocks evaluate to true are exclusive within a \Piecewise element. As in \MathML, the onus is on the model designer to ensure that the conditions do not overlap. For combinations of parameters and states that are not covered by any \Piece\withelem{}\Condition block, the \Piecewise function evaluates to the value of the \Otherwise\withelem{}\MathInline expression.

\note{Piecewise expressions do not appear within \Dynamics{}\withelem{}\Regime elements. Discontinuous expressions within \Dynamics blocks, require transitions to a new \Dynamics{}\withelem{}\Regime.}
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{Piece Structure}}}\\
    \toprule
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \MathInline & singleton & yes \\
    \Condition & singleton & yes \\    
    \bottomrule
  \end{edtable}
\end{table}

\Piece elements represent a continuous section of a piecewise function defined by the expression in \MathInline sub-element over the range of conditions that the \Condition block evaluates to true.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{Condition Structure}}}\\
    \toprule
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \MathInline & singleton & yes \\
...
The \MathInline expression within \Condition element must be evaluate to a boolean. It determines when the enclosing \Piece of the \Piecewise expression is active.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{Otherwise Structure}}}\\
    \toprule
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \MathInline & singleton & yes \\
...
The \Otherwise block is active within a \Piecewise expression whenever all other \Piece{}\withelem{}\Condition blocks within the \Piecewise element evaluate to false.
...
other during a simulation. Ports can either transmit discrete events (see \EventSendPort and \EventReceivePort), continuous streams of analog data (see \AnalogSendPort, \AnalogReceivePort and \AnalogReducePort), properties of a component (see \PropertySendPort and \PropertyReceivePort) or the indices of a cells, connections or compartments within a container (see \IndexSendPort and \IndexReceivePort).

\draftnote{see \href{https://github.com/INCF/nineml/issues/73}{GitHub Issue \#73} for discussion on Property and Index ports}
Events are typically used to model synaptic transmission but can be used more generally to model any instantaneous communication between components. Analog ports are used to model continuous communication between components, typically injected current, gap junctions or internal quantities shared between sub-components of a multi-component model. Property ports are used in several distinct situations: in order to pass parameters of cells in a population (such as their spatial coordinates) to \ConnectionRule or \Distribution components, to return properties from \Distribution components and to read properties of neighbouring compartments in multi-compartmental models.

For each mode of communication there is a sending a receiving port. For dynamic ports, which handle events and analog data streams, the send and receive ports are connected directly to each other. Whereas the \PropertySendPort and \PropertyReceivePort{}s are connected to \DerivedProperty and \Property elements respectively. \IndexReceivePort{}s receive connections from sources that are implicit to the container, whereas \IndexSendPort{}s are used in \Distribution elements used to generate multi-compartmental models.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    name & [\StateVariable | \Alias | \RandomVariable{}]@name & yes\\
    dimension & \Dimension{}@name & yes\\
    \bottomrule
  \end{edtable}
\end{table}
...
\subsection{PropertySendPort}
\label{sec:PropertySendPort}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{PropertySendPort Structure}}}\\
    \toprule
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    name & \Alias{}@name & yes\\
...
\PropertySendPort{}s are used to return quantities from \Component\withelem\Distribution{}s to populate \DerivedProperty elements.

\subsubsection{Name attribute}
Each \PropertySendPort requires a \textit{name} attribute, which is a valid \identifier and uniquely identifies the \PropertySendPort from all other elements in the \ComponentClass.

\subsubsection{Dimension attribute}
Each \PropertySendPort requires a \textit{dimension} attribute. This attribute specifies the dimension of the units of the quantity that is expected to be communicated through the \PropertySendPort and should refer to the name of a \Dimension element in the global scope.

\subsection{PropertyReceivePort}
\label{sec:PropertyReceivePort}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{PropertyReceivePort Structure}}}\\
    \toprule
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    name & \identifier & yes\\
...
\PropertyReceivePort{}s are used to read \Property{}s from 

\subsubsection{Name attribute}
Each \PropertyReceivePort requires a \textit{name} attribute, which is a valid \identifier and uniquely identifies the \PropertyReceivePort from all other elements in the \ComponentClass.

\subsubsection{Dimension attribute}
Each \PropertyReceivePort requires a \textit{dimension} attribute. This attribute specifies the dimension of the units of the quantity that is expected to be communicated through the \PropertyReceivePort and should refer to the name of a \Dimension element in the global scope.


\subsection{IndexSendPort}
\label{sec:IndexSendPort}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{IndexSendPort Structure}}}\\
    \toprule
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    name & \identifier & yes\\
...
\IndexSendPort{}s are used to return indices generated within \ComponentClass\withelem{}\Distribution elements, which can be used in \MultiCompartmental models to generate branch structures and domain-to-compartment mappings. Before sending, real numbers are floored to the nearest integer (\MathInline expressions provide, 

\subsubsection{Name attribute}
Each \IndexSendPort requires a \textit{name} attribute, which is a valid \identifier and uniquely identifies the \IndexSendPort from all other elements in the \ComponentClass.

\subsection{IndexReceivePort}
\label{sec:IndexReceivePort}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{IndexReceivePort Structure}}}\\
    \toprule
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    name & \identifier & yes\\
...
\end{table}
 ...
 \IndexReceivePort{}s can receive connections from sources that are implicit to the container such as the index of a cell, connection or compartment. The integer indices are implicitly converted to real numbers, which can then be used in \MathInline expressions.

\subsubsection{Name attribute}
Each \IndexReceivePort requires a \textit{name} attribute, which is a valid \identifier and uniquely identifies the \IndexReceivePort from all other elements in the \ComponentClass.
...
change (propagate in time) between subsequent transitions. \Regime elements 
...
\StateVariable, \AnalogReceivePort, \AnalogReducePort, \Parameter and {\Alias} elements. The \OnCondition block is triggered when the boolean result of the \Trigger statement changes from false to true. 
...
\draftnote{See \href{https://github.com/INCF/nineml/issues/72}{GitHub issue \#72}}
Network connectivity is defined via extensible components to promote reuse of connection logic. Connections are provisionally connected if the \ConnectCondition evaluates to true for a given connection. If \ConnectCondition is omitted then all possible connections are connected (i.e. all-to-all).  If a \SelectConnections element is provided the set of provisional connections will be pruned to a reduced size, which can be fixed or derived from other values. 
%Additionally, connections can be specified explicitly via \ExplicitConnections, which can either be in addition to the selected connections or replace the least preferred connections.

The expressions for \ConnectCondition and \SelectConnections can be derived from the indices of, or properties received from either the source or destination populations, or the projection itself (see \Projection for the way properties and indices of the projection are mapped onto source and destination cell pairs).
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \ConnectCondition & singleton & no\\
    \SelectConnections & set & no\\
%    \ExplicitConnections & singleton & no\\
    \Alias & set & no\\
    \Constant & set & no\\
    \RandomVariable & set & no\\
    \Piecewise & set & no\\    
...
\ConnectionRule is the top-level element for network connectivity component classes. 

\subsection{ConnectCondition}
\label{sec:ConnectCondition}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{ConnectCondition Structure}}}\\
    \toprule
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \MathInline & singleton & yes \\
...
\ConnectCondition elements specify the conditions under which a source and destination cell should be connected. If the provided \MathInline expression in it evaluates to true then the cells are provisionally connected (but can be pruned later as specified by the \SelectConnections element).
\draftnote{For explicit connection masks to be practical we will probably need a sparse format for array values. However, this is something we would probably want anyway I imagine (for accompanying connection weights for example).}
\note{Explicit connections between specific source and destination cells can be specified via a binary mask parameter passed directly to the \ConnectCondition expression.}

\subsection{SelectConnections}
\label{sec:SelectConnections}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{SelectConnections Structure}}}\\
    \toprule    
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    from & `fanOut' | `fanIn' | `total' & yes\\
    index & \primtype{non-negative integer} & yes\\
    \midrule
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \Number & singleton & yes \\
    \Preference & singleton & no\\
    \bottomrule
  \end{edtable}
\end{table}

\SelectConnections is used to select a subset of the provisionally connected connections (see \ConnectCondition). The number of connections to be selected is determined by the expression in the \Number element. Which connections are selected is determined by their rank as specified by the \Preference element. If no \Preference element is connected then the connections are selected at random. If there are insufficient provisional connections for the number requested, then all provisional connections are accepted but no additional connections are made.

The selections can be made from one of three different ``perspectives'': that of a cell in the source population, which fixes the number of connections that originate from single source cell (`fanOut'), that of a cell in the destination population, which fixes the number connections that are received by a single destination cell (`fanIn'), and that of the `total' number of connections in the projection. 


\subsubsection{From attribute}
Each \Component requires a \textit{from} attribute, which specifies the set of connections to be selected from. This can be the connections from each source sell, `fanOut', the connections to each destination cell, `fanIn', or all connections in the population `total'.

\subsubsection{Index attribute}
The \textit{index} attribute specifies the order in which multiple \SelectConnections are performed. It must be non-negative, unique amongst the set of \SelectConnections{}@index in the \ComponentClass{}\withelem{}\ConnectionRule, and the set of indices must be contiguous.
...
The number of cells in the population is specified by a \MathInline expression that must evaluate to a integer value. The onus is on the model designer to ensure the expression always evaluates to an integer value.
%
%\note{Built-in \primtype{real number}$\to$\primtype{integer} conversion functions, i.e. `\verb|ceil|', `\verb|floor|' and `\verb|round|' can be used to ensure the expression in the \Number element always evaluates to an integer.}
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{Preference Structure}}}\\
    \toprule
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \MathInline & singleton & yes \\     
...
Which subset to select from the provisional connections is determined by the \MathInline expression in the \Preference element, which can evaluate to any real number. Provisional connections are selected in order of preference from lowest to highest. Within subsets of equal preference, connections are selected in a random order.

%
%\subsection{ExplicitConnections}
%\label{sec:ExplicitConnections}
%
%\begin{table}[H]
%  \begin{edtable}{tabular}{llr}
%    \toprule
%    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{ExplicitConnections Structure}}}\\
%    \toprule
%    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
%    \midrule
%    source & \Parameter{}@name & yes\\
%    destination & \Parameter{}@name & yes\\
%    inAddition & `yes' | `no' & yes\\
%    \bottomrule
%  \end{edtable}
%\end{table}
%
%
%\subsubsection{Source attribute}
%Each \ExplicitConnections requires a \textit{source} attribute, which should be refer to the name of a property.

\section{Distributions}
\draftnote{see \href{https://github.com/INCF/nineml/issues/63}{GitHub Issue \#63}}
\Distribution{}s define how the value of parameters varying over members of a container (\Population, \Projection or \MultiCompartmental). The values can be dependent on any combination of random variables, the properties of members of the containers (see \PropertyReceivePort) and their indices (see \IndexReceivePort). The values provided via \PropertyReceivePort{}s and \IndexReceivePort{}s are iterated over each member of the container and the expressions are reevaluated, drawing new instances of each \RandomVariable{}.

\Component{}\withelem{}\Distribution elements are connected to \DerivedProperty elements via \PropertySendPort{}s. A single distribution component can be used to populate multiple \DerivedProperty elements via multiple \PropertySendPort{}s, allowing the same random variable instance to be shared between properties. 
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{Distribution Structure}}}\\
    \toprule
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \Alias & set & no\\
...
\Distribution is the top-level element for distribution component classes and defines all expressions.
...
\draftnote{This would be a bit nicer if we split the user layer into composition and parameterisation layers as then \Prototype{}s could be in the parameterisation layer and not have any \ReceiveConnection{}s, which don't really make sense in a prototype.}
...
document or in another file if a \textit{url} attribute is provided. Selected properties provided by the \Prototype\pointto\Component are then overridden by the referencing \Component.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \SingleValue | \ArrayValue | \ExternalArrayValue & singleton & yes \\ 
    \bottomrule
  \end{edtable}
\end{table}

\Property elements provide values for the parameters defined in the \ComponentClass
of the \Component. Their \textit{name} attribute should match the name of the corresponding \Parameter element in the \ComponentClass. The \Property should provide units that match the dimensionality of the corresponding \Parameter definition.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \FromDistribution & singleton & yes \\ 
    \bottomrule
  \end{edtable}
\end{table}

\DerivedProperty elements provide values for the parameters defined in the \ComponentClass
of the \Component that are derived from \PropertySendPort{}s of components. Their \textit{name} attribute should match the name of the corresponding \Parameter element in the \ComponentClass. 

\note{units are not provided to derived properties as they are determined from the component they are derived from}
...
The \ReceiveConnection element specifies a receive port which will receive a connection from a port of a remote component or container. Depending on the type of the \Component{}\withelem{}\ComponentClass the source of remote port to be connected can vary and are specified by different ``port source'' objects, e.g. \FromSource, \FromDestination and \FromSynapse  in \Projection containers.

\subsubsection{Port attribute}
Each \ReceiveConnection element requires a \textit{port} attribute. This should refer to the name of a port in the class definition of the component.

\subsection{FromDistribution}
\label{sec:FromDistribution}
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    port & {\PropertySendPort}@name & yes\\
    \midrule
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \Component{}\withelem\Distribution | \Reference{}\pointto\Component{}\withelem\Distribution & singleton & yes \\
    \bottomrule
  \end{edtable}
\end{table}

The values of \DerivedProperty elements are populated from \PropertySendPort{}s of \Component{}\withelem\Distribution{}s, as specified by \FromDistribution elements. The function is evaluated for each item in the container (i.e. \Population, \Projection or \MultiCompartmental) it is enclosed by.

\subsubsection{Port attribute}
Each \FromDistribution requires a \textit{port} attribute, which selects which port of the \Component to read.

\subsection{FromIndex}
\label{sec:FromIndex}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{FromIndex Structure}}}\\
    \toprule
     \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    of & `cell' | `source' | `destination' | `connection' | `sourceDomain' | 'destinationDomain' & yes\\
    \bottomrule
  \end{edtable}
\end{table}

\draftnote{Not quite happy with this either. Was thinking of reusing \FromSource, \FromDestination, etc... to provide the indices instead}
The \FromIndex element provides various indices for use in \ConnectionRule and \Distribution components. The indices that are available to use varies depending on the container the \FromIndex is located in. 

\subsubsection{Of attribute}
The \textit{of} attribute specifies which index should be supplied. Within a \Population, the only index available is that of the `cell'. For any element in a \Projection the `source', `destination' and `connection' indices are available. Whereas `sourceDomain' and `destinationDomain' are only valid in the \CompartmentConnectivity block and refer to the domain index of the source and destination compartments to determine the connectivity of.
...
In NineML, ``values'' are arrays that implicitly grow to fill the size of the container (i.e. \Population or \Projection) they are located within. Where located within \Property elements, values must represent \primtype{real numbers}, whereas values located within \Size, \Source, \Destination, \Branches and \Mapping elements they must be \primtype{integers}. Values can be one of three types:
\begin{itemize}
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Text format} & \; & \em{Required} \\
    \midrule
    \primtype{real number} | \primtype{integer} & \; & yes \\ 
    \bottomrule
  \end{edtable}
\end{table}
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Text format} & \; & \em{Required} \\
    \midrule
    \primtype{real number | integer} & \; & yes\\
    \bottomrule
  \end{edtable}
\end{table}
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \Property & set & no \\
    \DerivedProperty & set & no \\
    \Component{}\withelem{}\Distribution & set & no\\
    \bottomrule
  \end{edtable}
\end{table}

\draftnote{see \href{https://github.com/INCF/nineml/issues/19}{GitHub issue \#19} for a discussion around the use of \Property and \DerivedProperty directly within the \Population.}
A \Population defines a set of dynamic components of the same class. The size of the set is specified by the \Size element. The properties of the dynamic components are either specified directly via single or array values or derived from other \Component{}\withelem{}\Distribution{}s. \Component{}\withelem{}\Distribution{}s included within the \Projection container are evaluated once for each member in the container so that multiple \DerivedProperty elements can reference (see \Reference) the same random variable instance for each member.

\Property, \DerivedProperty and \Component{}\withelem{}\Distribution elements can be used to store additional properties of the cells, which are not related to their dynamics, but are useful for deriving connection rules, e.g. their spatial coordinates.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \FromDistribution & singleton & no \\
    \midrule    
    \em{Text format} & \; & \em{Required} \\
    \midrule
    \primtype{non-negative integer} & \; & no\\
    \bottomrule
  \end{edtable}
\end{table}

The number of cells in the population is specified either by a non-negative integer provided in the text of the \Size element or from an \IndexSendPort within a \Distribution component. The value of the \FromDistribution{}@port should not depend on any \IndexReceivePort or \PropertyReceivePort only the \Parameter{}s of the \Component{}\withelem{}\Distribution. 
\draftnote{This is a little bit messy, any suggestions? I am thinking of creating a ``SizeSendPort'' but this seems a little strange}

It is envsaged that deriving the size of the population from a \Component{}\withelem{}\Distribution port would be useful when parameters to the spatial layout of the population (e.g. length, width and depth) determines the total number of cells in the population.

\subsubsection{Text format}
The text of the \Size element should contain an \primtype{integer} representing the size of the population if and only if it isn't derived from a \FromDistribution{}\withelem\IndexSendPort port. 
...
Array values passed to properties of the projection (e.g. synaptic weights and delays) can either be the size of the number of connections made or the number of possible unique connections (i.e. $N_{\mathrm{source}} * N_{\mathrm{dest}}$). In either case, the expected array data is ``destination population major'' (like \href{http://en.wikipedia.org/wiki/Row-major_order}{``row major order'', http://en.wikipedia.org/wiki/Row-major\_order}), i.e. it first iterates the destination population then the source population,
\begin{equation}
i_{\mathrm{connection}} =  i_{\mathrm{destination}} + i_{\mathrm{source}} * N_{\mathrm{source}}
\end{equation}
where $N$ is the size of the population and $i$ the index of the cell/connection (starting from 0). For the case of the explicit arrays being the size of the number connections made, data for $i_{\mathrm{value}}$ indices that do not correspond to connected pairs are omitted.
...
The \Projection element contains all the elements that define a projection between two populations and should be uniquely identified in the scope of the document. \Component{}\withelem{}\Distribution{}s included within the \Projection container are evaluated once for each member in the container so that multiple \DerivedProperty elements can reference (see \Reference) the same random variable instance for each member.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \Component{}\withelem\Dynamics & singleton & yes \\ 
    \bottomrule
  \end{edtable}
\end{table}
...
The \FromSource element specifies a port connection to the projection component (either the destination cell, post-synaptic response or plasticity dynamics) inside which it is inserted from the source cell dynamics. The delay on transmission of the port connection is specified by a \Delay element. If a \Delay element is not provided the delay is zero.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \toprule
     \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    port & [\AnalogSendPort | \EventSendPort{} | \Parameter{}]@name & yes\\
    \midrule
    \em{Element type} & Multiplicity & \em{Required} \\
    \midrule
    \Delay & singleton & no\\
...
The \FromDestination element specifies a port connection to the projection component (either the source cell, post-synaptic response or plasticity dynamics) inside which it is inserted from the destination cell dynamics. The delay on transmission of the port connection is specified by a \Delay element. If a \Delay element is not provided the delay is zero.
...
The \FromSynapse element specifies a port connection to the source or destination cell component from the \Synapse element. The delay on transmission of the port connection is specified by a \Delay element. If a \Delay element is not provided the delay is zero.
...
\subsection{DerivedDelay}
\label{sec:DerivedDelay}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{DerivedDelay Structure}}}\\
    \toprule
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \FromDistribution & singleton & yes \\ 
...
Sets of \Population and \Projection elements are collected into networks, which are the top-level NineML object model. \Network{}s can also consist of smaller sub-networks allowing complex networks to be built up incrementally.

\subsection{Network}
\label{sec:Network}

%\draftnote{could be omitted and just inferred from the source and destination references of the projections}
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{Network Structure}}}\\
    \toprule
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \Reference{}\pointto{}\Population & set & yes\\    
    \Reference{}\pointto{}\Projection & set & yes\\
    \Reference{}\pointto{}\Network & set & yes\\    
    \bottomrule
  \end{edtable}
\end{table}

The \Network object is the highest level NineML container.
...
\MultiComponent models can be flattened to a \Component\withelem{}\Dynamics and therefore can be used any place \Component\withelem{}\Dynamics elements are valid. 
\subsubsection{Name attribute}
Each \ComponentClass requires a \textit{name} attribute, which should be a valid \identifier and uniquely identify the {\ComponentClass} in the global scope.
...
\SubComponent elements can refer to any \ComponentClass\withelem\Dynamics class. They elements are explicitly connected to other \SubComponent elements via ports specified by \ReceiveConnection\withelem{}\FromSister elements. Apart from accepting \ReceiveConnection elements, \SubComponent elements are the defined the same as \Component elements.
...
\subsection{FromSister}
\label{sec:FromSister}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{FromSister Structure}}}\\
    \toprule
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    component & \SubComponent{}@name & yes\\
    port & [\EventSendPort{} | \AnalogSendPort{}]@name & yes\\
    \bottomrule
  \end{edtable}
\end{table}

The \FromSister element specifies a port connection to the enclosing \SubComponent component from another \SubComponent. 

\subsubsection{Component attribute}
Each \FromSister requires a \textit{component} attribute, which should refer to a \SubComponent within the \MultiComponent scope.

\subsubsection{Port attribute}
Each \FromSister requires a \textit{port} attribute, which should refer to the name of a port in the corresponding \SubComponent.
...
\PortExposure elements expose ports within \SubComponent elements outside the \MultiComponent. They are retained as normal ports in the flattened (to the core) version of the component.

\subsubsection{Name attribute}
Each \PortExposure requires a \textit{name} attribute, which should be a valid \identifier and uniquely identify the \PortExposure in the \MultiComponent scope.

\subsubsection{Component attribute}
Each \PortExposure requires a \textit{component} attribute, which should refer to a \SubComponent within the \MultiComponent scope.

\subsubsection{Port attribute}
Each \PortExposure requires a \textit{port} attribute, which should refer to the name of a port in the corresponding \SubComponent.

\section{Multi-compartmental models}
\draftnote{see \href{https://github.com/INCF/nineml/issues/30}{GitHub Issue \#30}}
Multi-compartmental models enable component classes with axonal and dendritic trees to be constructed by grouping compartments into different ``domains''. As with multi-component models, the multi-compartmental elements extend the User Layer.

The expected technique for flattening to multi-compartmental models to the core, would include first flattening to multi-component models. For implementation in simulators that don't provide structures for multi-compartmental modelling (e.g. NEST, Brian, etc...) flattened descriptions would be used. For simulators with explicit structures for multi-compartmental modelling (e.g. NEURON, GENESIS, etc...), the compartments can be translated directly to these simulator specific structures.

\note{This format does not contain any positional information about the compartments only their connectivity to one another. However, 3D points could be optionally included as annotations to allow plotting and other analysis but would not effect the length parameter in the dynamics component or the interpretation of the dynamics in general.}

\subsection{MultiCompartmental}
\label{sec:MultiCompartmental}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{MultiCompartmentClass Structure}}}\\
    \toprule
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    name & \identifier & yes\\
    ...
    \em{Element type} & Multiplicity & \em{Required} \\
    \midrule \relax
    \Branches & singleton & yes\\
    \Mapping & singleton & yes\\   
    \Domain & set & yes\\
    \MultiPortExposure & set & no\\
    \Property & set & no\\
    \DerivedProperty & set & no\\
    \Component{}\withelem{}\Distribution & set & no\\
    \bottomrule
...
The \MultiCompartmental is the top-level element for multi-compartmental models. It can be only used within \Population\withelem{}\Cell elements. The branching structure of the compartments (i.e. which compartments are connected to which) is specified by the \Branches element, while the dynamics of the compartments is specified in \Domain they are mapped to by the \Mapping element. 
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{Branches Structure}}}\\
    \toprule
    \em{Element type} & Multiplicity & \em{Required} \\
    \midrule \relax
    \SingleValue | \ArrayValue | \ExternalArrayValue | \FromDistribution & singleton & yes\\
    \bottomrule
  \end{edtable}
\end{table}

The Branches is an array containing the indices of the parent node of the segment. In the future instead of an ArrayValue (or ExternalArrayValue) this could be substituted for a component that generates the branch structure. The format is similar to the parent column in the SWC format with the caveat that the segment indices start from 0 and range to $N-1$, where $N$ is the number of compartments (i.e. $i=0,\ldots,N-1$).
...
The parameters of the components can either be SingleValues, ArrayValues or Components. Like Projections, the length of the arrays could either be the same as the number of compartments in the model, in which case only the values that map to the domain would be used, or the same as the number of compartments in the domain.

\subsubsection{Name attribute}
Each \Domain requires a \textit{name} attribute, which should be a valid \identifier and uniquely identify the \Domain in the \MultiCompartmental scope.

\subsubsection{Index attribute}
Each \Domain requires a \text{index} attribute, which should be a non-negative integer and unique amongst the \Domain element in the enclosing \MultiCompartmental. The \textit{index} is used to map the compartments onto the domain.

\subsection{MultiPortExposure}
\label{sec:MultiPortExposure}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{MultiPortExposure Structure}}}\\
    \toprule
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    name & \identifier & yes\\
...
\MultiPortExposure elements expose ports within \Domain elements outside the \MultiCompartmental. They are distinguished from \PortExposure elements because when the \MultiCompartmental is flattened multiple ports are created, one for each compartment of the specified \Domain, or all compartments if no \Domain is specified.

\subsubsection{Name attribute}
Each \MultiPortExposure requires a \textit{name} attribute, which should be a valid \identifier and uniquely identify the \MultiPortExposure in the \MultiComponent scope.

\subsubsection{Domain attribute}
Each \MultiPortExposure requires a \textit{domain} attribute, which should refer to a \Domain within the \MultiCompartmental scope.

\subsubsection{Port attribute}
Each \PortExposure requires a \textit{port} attribute, which should refer to the name of a port in the corresponding \Domain.
...
\subsubsection{Domain attribute}
Each \FromProximal can define a \textit{domain} attribute. If present the \textit{domain} attribute should refer to a \Domain within the \MultiCompartmental scope.

\subsubsection{Port attribute}
Each \FromProximal requires a \textit{port} attribute, which should refer to the name of a port in the corresponding \Domain element, or all \Domain elements if the \textit{domain} attribute is omitted.
...
\subsection{MultiCompartmentProjection}
\label{sec:MultiCompartmentProjection}
...
For connecting populations with \MultiCompartmental cell models, \MultiCompartmentProjection must be used. \MultiCompartmentProjection extends the \Projection element by including \CompartmentConnectivity connection-rule which determines which compartments projections are connected by synapses when two cells are connected. 

In flattening the \MultiCompartmentProjection elements to Core NineML, a single \MultiCompartmentProjection is split into a set of \Projection elements, one for each source/destination compartment pairing that has a non-empty set of connections.

\subsubsection{Name attribute}
Each \MultiCompartmentProjection requires a \textit{name} attribute, which should be a valid \identifier and uniquely identify the \MultiCompartmentProjection in the global scope.
...
The \CompartmentConnectivity element specifies the connections between source and destination compartments if the \Connectivity element of the \MultiCompartmentProjection specifies a connection between the source and destination cell models. 
