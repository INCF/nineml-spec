...
\draftnote{This the LEMS format (which is very similar to a format I found in an old version of the NineML spec), I personally would favour more descriptive attributes (and changing Dimension to UnitDimension)
 but it is probably not worth changing it, especially as these will almost always be referenced from a catalog file, thoughts?}
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{UnitDimension Structure}}}\\
    \toprule
...
Each \UnitDimension requires a \textit{name} attribute. This name should be a valid identifier in the \href{http://msdn.microsoft.com/en-us/library/e7f8y25b.aspx}{ANSI C89 standard} and uniquely identify the {\UnitDimension} in current the scope.

\subsubsection{m attribute}
The \textit{m} attribute specifies the power of the mass dimension in the \UnitDimension. If omitted it the power is one.

\subsubsection{l attribute}
The \textit{l} attribute specifies the power of the length dimension in the \UnitDimension. If omitted it the power is one.

\subsubsection{t attribute}
The \textit{t} attribute specifies the power of the time dimension in the \UnitDimension. If omitted it the power is one.

\subsubsection{i attribute}
The \textit{i} attribute specifies the power of the current dimension in the \UnitDimension. If omitted it the power is one.

\subsubsection{n attribute}
The \textit{n} attribute specifies the power of the amount-of-substance dimension in the \UnitDimension. If omitted it the power is one.

\subsubsection{k attribute}
The \textit{k} attribute specifies the power of the temperature dimension in the \UnitDimension. If omitted it the power is one.

\subsubsection{? attribute}
\draftnote{I can\rq{}t find the luminous intensity abbreviation}
The \textit{?} attribute specifies the power of the luminous-intensity dimension in the \UnitDimension. If omitted it the power is one.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{Units Structure}}}\\
    \toprule
    \em{Attribute name} & \em{Type/Format} & \em{Required} \\
    \midrule
    symbol & \href{http://msdn.microsoft.com/en-us/library/e7f8y25b.aspx}{ANSI C89 identifier} & yes\\
    dimension & \UnitDimension{}@name & yes\\
...
Each \Units requires a \textit{symbol} attribute. This name should be a valid identifier in the \href{http://msdn.microsoft.com/en-us/library/e7f8y25b.aspx}{ANSI C89 standard} and uniquely identify the {\Units} in current the scope.

\subsubsection{Dimension attribute}
Each \Units requires a \textit{dimension} attribute. This attribute specifies the dimension of the units and should refer to the name of a \UnitDimension element in the global scope.

\subsubsection{PowTen attribute}
Each \Units requires a \textit{powTen} attribute. This attribute specifies the relative scale of the units compared to the equivalent SI units in powers of ten.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \Reference->\Population & singleton & no \\
    ...
    \em{Text format} & \; & \em{Required} \\
    \midrule
    name:Population & \; & if \Reference is not provided\\
    \bottomrule
  \end{edtable}
\end{table}
