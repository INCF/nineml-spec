...
\draftnote{This the LEMS format (which is very similar to a format I found in an old version of the NineML spec). I personally would favour more descriptive attributes and changing Dimension to UnitDimension but I think this format should be fine}
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
...
\draftnote{Are {\Regime}s required? Could you have a component that simply consists of an \Alias?}
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
...
\draftnote{For consistency with the \Cell element, part of me wants to enforce the use of the \Reference element without a url attribute to refer to populations in the same file, although another part finds this unnecessarily verbose (theoretically then the population could be defined within the projection although I don\rq{}t know when you would want to do this in practice). }
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \Reference->\Population | \Reference->\Set & singleton & no \\
    ...
    \em{Text format} & \; & \em{Required} \\
    \midrule \relax
    [\Population| \Set{}]@name & \; & if \Reference is not provided\\
    \bottomrule
  \end{edtable}
\end{table}

\subsubsection{Text format}
If the source \Population is in the same file as the \Projection element it should be referenced by name in the text of the \Source element (otherwise a \Reference element should be used).
...
\subsubsection{Text format}
If the destination \Population is in the same file as the \Projection element it should be referenced by name in the text of the \Destination element (otherwise a \Reference element should be used).
...
\subsubsection{Text format}
\draftnote{Again, I am not sure it is a good idea to make this optional (see note for \Definition)}
...
\draftnote{Set doesn\rq{}t seem to be the right name considering we are trying to conserve cell indices}
...
\subsubsection{Index attribute}
Each \Item requires a \textit{index} attribute. This attribute specifies the order in which the {\Population}s in the \Set are concatenated and thereby the indices of the cells within the combined \Set.
