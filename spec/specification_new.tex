...
\newcommand{\HomogenousValue}{\defRef{\textbf{\class{HomogenousValue}}\xspace}{sec:HomogenousValue}}
\newcommand{\RandomlyDistributedValues}{\defRef{\textbf{\class{RandomlyDistributedValues}}\xspace}{sec:RandomlyDistributedValues}}
...
\newcommand{\ListHeader}{\defRef{\textbf{\class{ListHeader}}\xspace}{sec:ListHeader}}
\newcommand{\ListRow}{\defRef{\textbf{\class{ListRow}}\xspace}{sec:ListRow}}
...
\newcommand{\CellType}{\defRef{\textbf{\class{CellType}}\xspace}{sec:CellType}}
...
Each \ProbabilisticConnectivity element requires a \textit{probability} attribute. This attribute should refer to a name of a dimensionless \Parameter in the enclosing \ComponentClass. The properties supplied to this parameter should either be a homogeneous value or a $M{\times}N$ list of probabilities where $M$ and $N$ are the size of the source and destination populations respectively.
...
Each \ExplicitConnectionList element requires a \textit{source-indices} attribute. This attribute should refer to a name of a dimensionless \Parameter in the enclosing \ComponentClass. The properties supplied to this parameter should be a list drawn from the set $\{1,\ldots,M\}$ where $M$ is the size of the source population and be the same length as the property supplied to the \textit{target-indices} parameter.

\subsubsection{Target-indices attribute}

Each \ExplicitConnectionList element requires a \textit{target-indices} attribute. This attribute should refer to a name of a dimensionless \Parameter in the enclosing \ComponentClass. The properties supplied to this parameter should be a list drawn from the set $\{1,\ldots,N\}$ where $N$ is the size of the source population and be the same length as the property supplied to the \textit{source-indices} parameter.
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \HomogenousValue | \Component->\RandomDistribution | \ValueList | \ExternalValueList & singleton & yes \\
    \bottomrule
  \end{edtable}
\end{table}
...
\subsection{HomogenousValue}
\label{sec:HomogenousValue}

\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    \toprule
    \multicolumn{3}{c}{\parbox{0.55\linewidth}{\center\textbf{HomogenousValue Structure}}}\\
    \toprule
    \em{Text} & \; & \em{Required} \\
    \midrule
    numeric-value & \; & yes \\ 
...
\begin{table}[H]
  \begin{edtable}{tabular}{llr}
    ...
    \em{Element type} & \em{Multiplicity} & \em{Required} \\
    \midrule
    \ListHeader & singleton & yes \\
    \ListRow & set & no \\
    \bottomrule
  \end{edtable}
\end{table}

\subsection{ListHeader}
\label{sec:ListHeader}

\subsection{ListRow}
\label{sec:ListRow}

\subsection{ListColumn}
\label{sec:ListColumn}
...
\item \EventOut {[}0+{]} (The events that should be sent when this
...
%
\begin{itemize}
\item \verb|name| {[}Required{]} Has to match a name of a \Parameter
or a \StateVariable in corresponding Abstraction Layer definition
\end{itemize}
